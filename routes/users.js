  ...
  User.findOne({
    ...
    .then(user => {
      ...
      // Send email with reset instructions & notify user
        ...
        req.flash(
          'success',
          'An email with instructions has been sent to the associated address'
        );
        res.redirect('/users/reset-request');
...
router.post('/changeInfo', (req, res) => {
  // Get params
  var { first_name, last_name, new_email, confirm_email_address } = req.body;
  // Update email
  if (
    new_email != req.user.email &&
    new_email.length > 0 &&
    new_email == confirm_email_address
  ) {
    // TODO
    req.flash(
      'success',
